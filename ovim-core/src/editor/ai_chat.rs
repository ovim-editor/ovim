...
impl Editor {
            ...
            match chunk {
                ...
                StreamChunk::Done => {
                    ...
                    if !tool_calls.is_empty() {
                        ...
                        // 2. Build execution context and execute tools
                        let exec_ctx = self.build_tool_execution_context();
                        for tc in &tool_calls {
                            let result = self.execute_tool_call(tc, &exec_ctx);
                        ...
                        if let Err(e) = self.continue_ai_chat_with_tools() {
                            if let Some(conv) = self.ai_state.conversations.get_mut(&key) {
    ...
    fn build_chat_capabilities(&self) -> Capabilities {
    ...
    fn build_tool_schemas_for_chat(
        &self,
    ...
    /// Execute a single tool call, checking scope before dispatch.
    ...
    /// Continue the chat after tool execution — start a new streaming request.
    fn continue_ai_chat_with_tools(&mut self) -> Result<()> {
        let chat = match self.ai_state.chat.as_ref() {
            Some(c) => c,
            None => return Ok(()),
        };
    ...
    fn get_diagnostics_for_current_buffer(&self) -> Vec<crate::ai::DiagnosticFact> {

...
pub(crate) fn explain_with_codebase_def() -> ToolDefinition {
    ToolDefinition {
        ...
        description: "Present a finished explanation as an ordered, interactive code walkthrough. The editor hides chat, expands code to the available width, highlights each inclusive range, and lets the user move between steps. Use it when understanding depends on seeing concrete code locations in sequence: for example, a requested codebase tour, an implementation flow across files, or an explanation of finished changes. Do not use it for a short answer about the current line, a simple factual question, or while implementation must continue; the walkthrough blocks until the user completes or dismisses it. Start with the user-visible goal or the simplest useful entry point, then establish each prerequisite before its consequence. Introduce unfamiliar terms only when the highlighted code needs them. Each step must add exactly one idea or relationship. If explaining a location requires two new ideas, make two steps, even when both steps highlight the same code. Keep the comment short and direct: state what this code establishes, why it matters, and only the single connection needed for the next step. Do not merely paraphrase syntax or front-load details used several steps later. Highlight the smallest contiguous block that proves the comment. Do not select an entire function, type, or large branch by default; select the exact condition, assignment, call, or small block doing the work. Use a single-line anchor for a handoff or invariant, and add end_line only when every selected line is needed for this one idea. You may revisit a line or range when a later step adds a genuinely different perspective; comparisons can deliberately move A to B to A to C. Validation never truncates ranges; an overflow error reports measured visual rows and retry guidance. Example: parser.rs:44-47 identifies the dispatch condition, store.rs:91 shows its handoff, parser.rs:52-55 shows the resulting state change, then parser_test.rs:126-130 proves that one behavior."
            .to_string(),
        required_scope: RequiredScope {
            file_scope: FileScope::Project,
            shell: false,
        ...
        parameters: vec![ToolParam {
            ...
            required: true,
            description: "Narratively ordered, small walkthrough steps. Paths are project-relative and lines are 1-indexed and inclusive. Give each step exactly one new idea and one necessary connection. If a comment needs two ideas, split it into two steps. Select the smallest code block that demonstrates the idea, not the surrounding function. Establish prerequisites before consequences. A line or range may be revisited when the later step adds a distinct insight."
                .to_string(),
        }],
    }
}

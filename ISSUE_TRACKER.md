...
| OV-00050 | Pending | HIGH | Medium | [RENDER] `slice_horizontal_viewport` uses char count instead of display width — compares `chars.len()` against terminal column `width`, but wide chars (CJK, emoji) occupy 2 display columns per char. Lines with wide chars overflow or get incorrectly sliced in nowrap mode. (ovim/src/ui/renderer/buffer.rs:24-69) |
| OV-00051 | Pending | HIGH | Medium | [RENDER] Visual selection and diagnostic underlines misaligned on lines with tabs — `render_line_with_highlights` iterates `col_idx` over tab-expanded text, but visual selection coords (`sel_start_col`/`sel_end_col`) and LSP diagnostic `character` offsets are in original-text space. Tab characters expand 1 char to N spaces, shifting all positions after the first tab. (ovim/src/ui/renderer/buffer.rs:806-864) |

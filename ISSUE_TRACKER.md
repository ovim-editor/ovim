...
| OV-00149 | Pending | HIGH | Medium | [LSP] Debounce timer flushes to single server, not broadcast — `process_flush_requests()` calls `flush_pending_changes()` (single server) instead of `flush_pending_changes_broadcast()`. Companion servers (e.g. clippy alongside rust-analyzer) never receive debounced `didChange` notifications. (ovim-core/src/lsp/notifications.rs:1111) |
| OV-00150 | Pending | HIGH | Medium | [PERF] Version lock held across `.await` in `send_did_change_immediate` — `document_versions` Mutex held during `server.notify().await` (network I/O). Blocks all document version operations while waiting for LSP server. Intentional for ordering but too broad — should increment+drop lock, then send. (ovim-core/src/lsp/notifications.rs:196-217) |
| OV-00151 | Pending | HIGH | Medium | [PERF] `merge_diagnostics()` called 2-3x per render frame under Mutex — `get_diagnostics()` and `count_diagnostics()` both acquire diagnostics lock and call `merge_diagnostics()` (O(n) with HashSet + clones). Called every frame from status line + gutter rendering. Should cache merged result. (ovim-core/src/lsp/mod.rs:432-512) |
...
| OV-00153 | Pending | MEDIUM | Low | [LSP] 3 request methods bypass `parse_lsp_response` helper — `inlay_hints` uses `unwrap_or_default()` (silent failure, no log). `document_symbols` and `workspace_symbols` try two parse paths with `result.clone()` and return empty vec with no log if both fail. (ovim-core/src/lsp/requests.rs:809,916,1285) |
| OV-00154 | Pending | MEDIUM | Low | [LSP] Health check uses `child.id()` instead of `try_wait()` — `id()` returns `Some(pid)` even for exited/zombie processes. Health check can report dead servers as alive, contradicting `Failed` state from reader EOF. (ovim-core/src/lsp/server.rs:1423-1430) |
| OV-00155 | Pending | MEDIUM | Low | [LSP] Initialize timeout leaves orphaned pending request — if `initialize()` times out, the pending request stays in `pending_requests` map until stale cleanup (10 min). Repeated timeouts accumulate toward MAX_PENDING_REQUESTS limit. (ovim-core/src/lsp/server.rs:687-703) |
| OV-00156 | Pending | MEDIUM | Low | [LSP] `ShuttingDown` state not rejected in request gate — `request()` checks for `Failed` and `Terminated` states but not `ShuttingDown`. Requests sent during shutdown hang until 10s timeout instead of failing immediately. (ovim-core/src/lsp/server.rs:1144-1154) |
| OV-00157 | Pending | MEDIUM | Low | [LSP] `pending_completion` not cleared on buffer switch — `clear_lsp_state()` aborts goto/hover responses but doesn't clear `pending_completion`. Stale completion response could theoretically be applied after file switch (mitigated by insert mode check). (ovim-core/src/editor/lsp_integration.rs:556-565) |

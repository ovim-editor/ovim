...
| OV-00139 | Pending | LOW | Low | [MOTION] `section_forward()` and `section_end_forward()` use `rope.len_lines()` instead of `buffer.line_count()` — may search phantom empty line at end of buffer. Other motions use `line_count()` for Vim-compatible counting. (ovim-core/src/editor/motions.rs:1481, ovim-core/src/editor/motions.rs:1538) |

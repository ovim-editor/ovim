...
pub fn expand_tabs_with_mapping(text: &str, tab_width: usize) -> TabExpansion {
...
pub fn compose_conceal_and_tabs(
    ...
    let inner = expand_tabs_with_mapping(&transform.text, tab_width);
    let expanded = inner.text;
    let byte_map_after = inner.byte_mapping;
    let control = inner.control_ranges;
    let char_map_after = inner.char_mapping;
...
mod tests {
    ...
    fn test_expand_tabs_basic() {
        let exp = expand_tabs_with_mapping("\thello", 4);
        assert_eq!(exp.text, "    hello");
        assert!(exp.control_ranges.is_empty());
    }

    #[test]
    fn test_expand_control_chars() {
        // ESC (0x1B) → ^[
        let exp = expand_tabs_with_mapping("\x1b[31m", 4);
        assert_eq!(exp.text, "^[[31m");
        assert_eq!(exp.control_ranges.len(), 1);
        assert_eq!(exp.control_ranges[0], 0..2); // ^[ occupies bytes 0..2
    }

    #[test]
    fn test_expand_nul() {
        let exp = expand_tabs_with_mapping("a\x00b", 4);
        assert_eq!(exp.text, "a^@b");
        assert_eq!(exp.control_ranges.len(), 1);
        assert_eq!(exp.control_ranges[0], 1..3); // ^@ at byte offset 1..3
    }

    #[test]
    fn test_expand_del() {
        let exp = expand_tabs_with_mapping("x\x7fy", 4);
        assert_eq!(exp.text, "x^?y");
        assert_eq!(exp.control_ranges.len(), 1);
        assert_eq!(exp.control_ranges[0], 1..3);
    }

    #[test]
    fn test_expand_multiple_control_chars() {
        // "\x1b[31mred\x1b[0m" → "^[[31mred^[[0m"
        let exp = expand_tabs_with_mapping("\x1b[31mred\x1b[0m", 4);
        assert_eq!(exp.text, "^[[31mred^[[0m");
        assert_eq!(exp.control_ranges.len(), 2);
        assert_eq!(exp.control_ranges[0], 0..2); // first ^[
        assert_eq!(exp.control_ranges[1], 9..11); // second ^[
    }

    #[test]
    fn test_expand_no_control_chars() {
        let exp = expand_tabs_with_mapping("hello world", 4);
    ...
    fn test_byte_mapping_with_control_chars() {
        // "a\x01b" → "a^Ab"
        let exp = expand_tabs_with_mapping("a\x01b", 4);
        assert_eq!(exp.text, "a^Ab");
        // mapping: orig 0 → exp 0 ('a'), orig 1 → exp 1 ('\x01' → ^A), orig 2 → exp 3 ('b'), end
        assert_eq!(exp.byte_mapping[0], (0, 0));
        assert_eq!(exp.byte_mapping[1], (1, 1));
        assert_eq!(exp.byte_mapping[2], (2, 3));
        assert_eq!(exp.byte_mapping[3], (3, 4)); // end sentinel
    }
    ...
    fn test_char_mapping_with_tabs() {
        ...
        assert_eq!(exp.char_mapping[0], 0);
        // original char 1 ('h') → expanded char 4
        assert_eq!(exp.char_mapping[1], 4);
        // original char 2 ('e') → expanded char 5
        assert_eq!(exp.char_mapping[2], 5);
        // sentinel
        assert_eq!(exp.char_mapping[6], 9);
    }
    ...
    fn test_char_mapping_with_control_chars() {
        // "a\x01b" → "a^Ab"
        let exp = expand_tabs_with_mapping("a\x01b", 4);
        assert_eq!(exp.char_mapping[0], 0); // 'a' → expanded 0
        assert_eq!(exp.char_mapping[1], 1); // '\x01' → expanded 1 (^A takes 2 chars)
        assert_eq!(exp.char_mapping[2], 3); // 'b' → expanded 3
        assert_eq!(exp.char_mapping[3], 4); // sentinel
    }
    ...
    fn test_char_mapping_plain_text() {
        let exp = expand_tabs_with_mapping("hello", 4);
        for i in 0..=5 {
            assert_eq!(exp.char_mapping[i], i); // 1:1 mapping for plain text
        }
    }
    ...
    fn test_remap_char_col() {
        let exp = expand_tabs_with_mapping("\thello", 4);
        assert_eq!(remap_char_col(0, &exp.char_mapping), 0);
        assert_eq!(remap_char_col(1, &exp.char_mapping), 4);
        assert_eq!(remap_char_col(2, &exp.char_mapping), 5);
        // Beyond range returns sentinel
        assert_eq!(remap_char_col(100, &exp.char_mapping), 9);
    }

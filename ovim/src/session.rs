        ...
        let is_autogenerated = |name: &str| {
            ...
            parts.len() >= 3
                && parts
                    .iter()
                    .skip(1)
                    .all(|p| p.chars().all(char::is_numeric))
        };

        // Separate TUI (auto-generated) from named sessions
        let (auto_sessions, named_sessions): (Vec<_>, Vec<_>) = sessions
            .into_iter()
            .partition(|s| is_autogenerated(&s.session_name));

        // Prefer named sessions (user explicitly named them)
        if let Some(session) = named_sessions.into_iter().max_by_key(|s| s.started_at) {
            return Ok(session);
        }

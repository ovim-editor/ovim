...
Object.assign(mockSnapshot.aiChat, {
    ...
    codeExplanation: undefined, setup: undefined
        ...
        if (state === "approval") {
            ...
            await expect(page.getByPlaceholder("Answer Claude Code’s question…")).toBeVisible();
        ...
        await page.getByRole("button", { name: /claude_code.*default/i }).click();
        await expect(page.getByRole("option", { name: /codex_sol/i })).toBeVisible();
        await expect(page.getByRole("option", { name: /claude_code/i })).toBeVisible();
    });
}

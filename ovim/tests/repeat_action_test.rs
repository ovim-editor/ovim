...
fn test_dedent_cursor_beyond_new_line_length() {
    ...
    assert!(after.1 <= line_len, "Cursor col {} exceeds line length {}", after.1, line_len);
}
...
fn test_indent_multiline_near_eof() {
    ...
    assert!(lines[2].starts_with("    "), "Last line should be indented: {:?}", lines[2]);
}
...
fn test_indent_multiline_dot_repeat_near_eof() {
    ...
    assert!(lines[3].starts_with("    "), "Line 3 should be indented by repeat");
}

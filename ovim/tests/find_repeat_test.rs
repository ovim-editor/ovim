...
fn test_find_forward_repeat() {
    ...
    assert_eq!(
        test.cursor(),
        (0, 7),
        "'h' in 'there' should be at column 7"
    );
    ...
    assert_eq!(
        test.cursor(),
        (0, 12),
        "'h' in second 'hello' should be at column 12"
    );
    ...
    assert_eq!(
        test.cursor(),
        (0, 24),
        "'h' in third 'hello' should be at column 24"
    );
}
...
fn test_find_repeat_opposite_direction() {
    ...
    assert_eq!(
        test.cursor(),
        (0, 0),
        "Should continue backward to 'h' at position 0"
    );
    ...
    assert_eq!(
        test.cursor(),
        (0, 0),
        "Should stay at position 0 (no more 'h' backward)"
    );
}
...
fn test_till_forward_repeat() {
    ...
    assert_eq!(
        test.cursor(),
        (0, 3),
        "Should be one before 'e' at column 3"
    );
    ...
    assert_eq!(
        test.cursor(),
        (0, 15),
        "Should be one before 'o' at column 15"
    );
    ...
    assert_eq!(
        test.cursor(),
        (0, 15),
        "Should stay at column 15 (no more 'o')"
    );
}
...
fn test_till_backward_repeat() {
    ...
    assert_eq!(
        test.cursor(),
        (0, 1),
        "Should stay at column 1 (no more 'a')"
    );
}

...
/// Create an editor with clipboard disabled to avoid cross-test interference
/// via the process-wide system clipboard.
fn editor_with_content(content: &str) -> Editor {
...
fn test_line_paste_after_undo() {
    ...
    let mut editor = editor_with_content("line 1\nline 2\nline 3");
...
fn test_character_paste_after_undo() {
    ...
    let mut editor = editor_with_content("hello world test");
...
fn test_multiple_paste_undo() {
    ...
    let mut editor = editor_with_content("x");

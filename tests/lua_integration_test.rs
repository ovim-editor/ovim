...
fn test_lua_basic_execution() {
    ...
    let result = editor.execute_lua("return 2 + 2").expect("Failed to execute Lua");
    assert_eq!(result, "4");

    // Test string return
    let result = editor.execute_lua("return 'hello'").expect("Failed to execute Lua");
...
fn test_vim_fn_line() {
    ...
    let result = editor.execute_lua("return vim.fn.line('.')").expect("Failed to execute");
    assert_eq!(result, "1");
}
...
fn test_vim_fn_col() {
    ...
    let result = editor.execute_lua("return vim.fn.col('.')").expect("Failed to execute");
    assert_eq!(result, "1");
}
...
fn test_vim_api_get_current_line() {
    ...
    // Get current line
    let result = editor.execute_lua("return vim.api.nvim_get_current_line()").expect("Failed to execute");
...
fn test_vim_cmd_queues_command() {
    ...
    editor.execute_lua("vim.cmd('nohl')").expect("Failed to execute Lua");

    // Process the queued commands
    editor.process_lua_commands().expect("Failed to process commands");
...
fn test_multiple_lua_calls() {
    ...
    // Multiple Lua calls
    let r1 = editor.execute_lua("return vim.fn.line('.')").expect("Failed");
    let r2 = editor.execute_lua("return vim.fn.line('$')").expect("Failed");
...
fn test_lua_table_creation() {
    ...
    let result = editor.execute_lua("local t = {1, 2, 3}; return t[1]").expect("Failed");
    assert_eq!(result, "1");
}

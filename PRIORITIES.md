...
- [x] Added macro regression coverage for char-motion changes (`cf`/`ct`) undo+redo isolation, including no-insert `cfo<Esc>`/`ctX<Esc>` and `/search ... n.` dot-repeat chains.

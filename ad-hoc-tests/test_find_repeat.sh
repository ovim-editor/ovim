...
curl -s -X PUT "$API_URL/buffer" -H "Content-Type: application/json" \
  -d '{"content": "hello there hello world hello again"}' > /dev/null

# Move to start
curl -s -X POST "$API_URL/keys" -H "Content-Type: application/json" \
  -d '{"keys": "gg0"}' > /dev/null

echo "Initial buffer: 'hello there hello world hello again'"
CURSOR=$(curl -s "$API_URL/cursor" | jq -r '"\(.line),\(.column)"')
...
curl -s "$API_URL/snapshot" | jq '{mode: .mode, cursor: .cursor, buffer_preview: (.buffer.content[0:60])}'

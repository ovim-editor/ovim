...
response=$(curl -s $API_URL/v1/snapshot)
echo "Response:"
...
response=$(curl -s $API_URL/v1/buffer)
echo "Response:"
pretty_json "$response"
echo ""
...
response=$(curl -s $API_URL/v1/cursor)
echo "Response:"
pretty_json "$response"
echo ""
...
response=$(curl -s $API_URL/v1/mode)
echo "Response:"
pretty_json "$response"
echo ""
...
response=$(curl -s -X PUT $API_URL/v1/buffer \
...
response=$(curl -s -X POST $API_URL/v1/command \
  -H "Content-Type: application/json" \
  -d '{"command": "w"}')
echo "Response:"
pretty_json "$response"
...
curl -s -X POST $API_URL/v1/keys \
  -H "Content-Type: application/json" \
  -d '{"keys": "gg"}' > /dev/null
before=$(curl -s $API_URL/v1/cursor)
response=$(curl -s -X POST $API_URL/v1/keys \
  -H "Content-Type: application/json" \
  -d '{"keys": "5j"}')
after=$(curl -s $API_URL/v1/cursor)
